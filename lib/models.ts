...
export type ModelCapability = {
  ...
  mediaRules: { allowedRoles: MediaRole[]; maxByRole: Partial<Record<MediaRole, number>>; combinations: MediaRole[][]; allowMixedReferenceInputs?: boolean; maxTotalInputs?: number };
...
const MODELS: ModelCapability[] = [
  ...
  withTasks({ id: "alibaba:wan3-video", label: "Wan 3 Video", providerModel: "wan3.0-video", family: "wan3", media: "video", tasks: ["text-to-video", "image-to-video", "reference-to-video"], mediaRules: { allowedRoles: videoRoles, maxByRole: wan3VideoLimits, combinations: videoCombinations, allowMixedReferenceInputs: true, maxTotalInputs: 20 }, duration: { min: 2, max: 30, smartValue: -1 }, resolutions: ["480P", "720P", "1080P"], aspectRatios: wan3Ratios, defaultOptions: { duration: 5, resolution: "720P", aspectRatio: "adaptive", promptExtend: true, watermark: false, audio: false }, quotaRequired: true, status: "requires-confirmation" }),
...
export function validateGeneration(model: ModelCapability, request: { task: GenerationTask; prompt: string; inputRoles: InputRole[]; options: Record<string, unknown>; freeQuotaConfirmed?: boolean }) {
  ...
  if (hasFrameInput && hasReferenceInput) throw new Error(`${model.label} cannot mix frame and reference inputs.`);
  ...
  const isReferenceCombination = model.mediaRules.allowMixedReferenceInputs && hasReferenceInput && roles.every((role) => role.startsWith("reference-"));
